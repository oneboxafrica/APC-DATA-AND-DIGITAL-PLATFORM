...
export default function Navbar({ onMenuToggle }: NavbarProps) {
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-white lg:border-b shadow-sm h-14 px-3 md:px-10 lg:px-8 xl:px-10 flex items-center justify-between">
      <div className="flex items-center gap-2 md:gap-4">
        <button title="button" className="lg:hidden p-2 rounded-full hover:bg-gray-100" onClick={onMenuToggle}>
          <Menu className="h-6 w-6" />
        </button>
        <div className="text-blue-600 text-xl md:text-2xl font-bold cursor-pointer">facebook</div>
      ...
      <div className="flex items-center gap-3">
        <div className="hidden md:flex items-center gap-2 cursor-pointer">
          ...
          <span className="font-medium text-sm hidden lg:inline">James</span>
